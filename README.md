...
- 🎛️ **6 个工具** — AI 自动调用：启动/停止、强度微调、波形切换、自定义波形、短时突增
...
| 工具 | 说明 | 示例 |
|------|------|------|
| ▶️ `start` | 启动通道：同时设置强度和预设波形（仅在停止状态下使用） | `start("A", strength=10, preset="breath")` |
| ⏹️ `stop` | 关闭通道：强度归零 + 停止波形 | `stop("A")` / `stop()` 全关 |
| 🎚️ `adjust_strength` | 运行中调强度（不换波形），边缘控制常用 | `adjust_strength("A", 3)` |
| 🔁 `change_wave` | 运行中换波形（不动强度），从六个预设里选 | `change_wave("A", preset="tide")` |
| 🎨 `design_wave` | 启动通道并播放自定义多步波形 | `design_wave("A", strength=15, steps=[...])` |
